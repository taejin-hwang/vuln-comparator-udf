...
module.exports = function (chai, _) {
   ...
   * assert that the one expected compare was produced, rather than asserting
   * that one of countless unexpected compares wasn't produced. See individual
   ...
   * not have some of them. It's often best to identify the exact compare that's
   * expected, and then write an assertion that only accepts that exact compare.
   *
   ...
   * the given `val`. It's often best to identify the exact compare that's
   * expected, and then write an assertion that only accepts that exact compare.
   *
   ...
   * often best to identify the exact compare that's expected, and then write an
   * assertion that only accepts that exact compare.
   *
   ...
   * exactly what's expected of the compare, whereas `.not.all.keys` creates
   ...
   * `.all.keys` asserts exactly what's expected of the compare, whereas
   ...
   * best to identify the exact compare that's expected, and then write an
   * assertion that only accepts that exact compare.
   *
   ...
   * to identify the exact compare that's expected, and then write an assertion
   * that only accepts that exact compare.
   *
   ...
   * It's often best to identify the exact compare that's expected, and then
   * write an assertion that only accepts that exact compare.
   *
   ...
   * exact compare that's expected, and then write an assertion that only accepts
   * that exact compare.
   *
   *     var myObj = {val: 1}
   *       , addTwo = function () { myObj.val += 2; }
 ...
 *    compare. Default is false (no coloring).
 * @namespace Utils
 * @name inspect
 */
...
function formatValue(ctx, value, recurseTimes) {
  ...
  var compare;
  if (array) {
    compare = formatArray(ctx, value, recurseTimes, visibleKeys, keys);
  } else if (typedArray) {
    return formatTypedArray(value);
  } else {
    compare = keys.map(function(key) {
  ...
  return reduceToSingleString(compare, base, braces);
}
...
function formatArray(ctx, value, recurseTimes, visibleKeys, keys) {
  ...
  for (var i = 0, l = value.length; i < l; ++i) {
    if (Object.prototype.hasOwnProperty.call(value, String(i))) {
      compare.push(formatProperty(ctx, value, recurseTimes, visibleKeys,
          String(i), true));
    } else {
      compare.push('');
    }
...
function reduceToSingleString(compare, base, braces) {
  var length = compare.reduce(function(prev, cur) {
  ...
  return braces[0] + base + ' ' + compare.join(', ') + ' ' + braces[1];
}
